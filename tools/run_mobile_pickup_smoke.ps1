...
$harnessRoot = Join-Path $tempRoot ('mouse-frontier-mobile-pickup-smoke-' + [guid]::NewGuid().ToString('N'))
$reportRoot = Join-Path $projectRoot '.stabilization'
$reportPath = if ($ReportPath) { $ExecutionContext.SessionState.Path.GetUnresolvedProviderPathFromPSPath($ReportPath) } else { Join-Path $reportRoot 'mobile-pickup-smoke.rpt' }
...
try {
    ...
    foreach ($name in @('main.lua','conf.lua')) {
        Copy-Item -LiteralPath (Join-Path $PSScriptRoot ('mobile-pickup-smoke\' + $name)) -Destination (Join-Path $harnessRoot $name)
    }
    ...
    $null = $process.Handle
    if (-not $process.WaitForExit($TimeoutSeconds * 1000)) { throw 'Mobile pickup validation timed out.' }
    $process.WaitForExit()
    $report = [IO.File]::ReadAllText($reportPath)
    $errors = [IO.File]::ReadAllText($errorPath)
    if ($process.ExitCode -ne 0 -or $report -notmatch 'MOBILE_PICKUP_OK') { throw ($report + $errors) }
    Write-Output $report.Trim()
    Write-Output ('Screenshots: ' + (Join-Path $env:APPDATA 'LOVE\mouse-frontier-mobile-pickup-smoke'))
}
finally {
    ...
    if ($resolved.StartsWith($tempRoot,[StringComparison]::OrdinalIgnoreCase) -and (Split-Path -Leaf $resolved).StartsWith('mouse-frontier-mobile-pickup-smoke-')) {
        foreach ($name in @('game','assets','sounds')) {
