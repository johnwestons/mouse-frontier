local function install(resolve,assign)
  assert(type(resolve)=="function","screen UI requires a dependency resolver")
  assert(type(assign)=="function","screen UI requires a dependency writer")
  local env=setmetatable({}, {
    ...
    __newindex=function(_,key,value)
      if not assign(key,value) then error("screen UI cannot assign "..tostring(key),2) end
    end
  })
  setfenv(install,env)
