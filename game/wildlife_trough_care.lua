...
function Wildlife.new(options)
    ...
    return Difficulty.apply({kind="wildlife-trough-care",location=options.location or 1,helpQuestId=options.helpQuestId,progress=progress,phase=progress.phase,
        failureMessage="The animals grew wary. Back away and try again after the area settles.",pauseMessage="Wildlife care paused. Your observations are saved."},options.location)
end
...
function Wildlife.choose(session,index)
    ...
    if session.phase==1 then
        ...
        session.progress.step=session.progress.step+1; if session.progress.step>session.rounds then return UI.advance(session,2,"Visitors identified. Clean the trough before adding fresh food.") end
    ...
    elseif session.phase==2 then
        ...
        if index~=expected then return UI.miss(session,"Remove old feed, scrub, then rinse so the trough stays sanitary.") end
        session.progress.step=session.progress.step+1; if session.progress.step>session.rounds then return UI.advance(session,3,"The trough is clean. Measure food for each visitor group.") end
...
function Wildlife.mousepressed(session,x,y) local i=UI.hit(UI.variantZones(Wildlife.zones,session),x,y); return i and Wildlife.choose(session,i) end
...
function Wildlife.audit()
    local function complete(location)
        local s=Wildlife.new({location=location,progress={}}); local ok=true
        for _=1,s.rounds do ok=ok and Wildlife.choose(s,trackTarget(s))=="progress" end
        for _=1,s.rounds do ok=ok and Wildlife.choose(s,((s.progress.step-1)%3)+1)=="progress" end
        local done; for _=1,s.rounds do done=Wildlife.choose(s,visitor(s).portion) end
        return s,ok and done=="complete"
    end
    local early,earlyOK=complete(3); local late,lateOK=complete(45); local restored=Wildlife.new({location=45,progress=late.progress})
    local bad=Wildlife.new({location=45,progress={}}); local failed; for _=1,bad.maximumMistakes do failed=Wildlife.choose(bad,trackTarget(bad)%3+1) end
    return {ready=earlyOK and lateOK and restored.phase==3 and restored.progress.step==late.rounds+1 and failed=="failed",stages=3,
        earlyDecisions=early.rounds*3,lateDecisions=late.rounds*3,lateMistakes=late.maximumMistakes,persistent=true,keyboard=true,touch=true,foodProtected=true,curve="wildlife-trough-v2"}
end
return Wildlife
