...
function CharacterAnimation.draw(sets,file,action,x,y,maxWidth,maxHeight,facing,phase,clock)
    local set=sets[file]; local animation=set and (set[action] or set.idle)
    ...
    local walkCorrection = (action == "walk" or action == "idle") and -1 or 1
    local drawFacing = (facing or 1) * (layCorrection and -1 or 1) * walkCorrection
