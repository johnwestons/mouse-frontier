...
function Mice.update(list, dt, location, settlements)
    for _, mouse in ipairs(list or {}) do
        mouse.t = (mouse.t or 0) + dt
        mouse.wait = (mouse.wait or 0) - dt
        if mouse.wait <= 0 or not mouse.targetX then
            local angle = love.math.random() * math.pi * 2
            local distance = love.math.random(30, 105)
            mouse.targetX, mouse.targetY = nearbyWalkable(
                mouse.homeX + math.cos(angle) * distance,
                mouse.homeY + math.sin(angle) * distance * .62,
        ...
        if mouse.moving then
            ...
            local step = math.min(distance, 24 * dt)
