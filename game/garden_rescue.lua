...
function Garden.new(options)
    ...
    return {kind="garden-rescue",location=options.location or 1,helpQuestId=options.helpQuestId,progress=progress,phase=progress.phase,maximumMistakes=3,
        failureMessage="The thorns shifted toward a seedling. Step back and plan another careful attempt.",pauseMessage="Garden rescue paused. Your marked plants are saved."}
end
...
function Garden.choose(session,index)
    ...
    if session.phase==1 then
        ...
        session.progress.step=session.progress.step+1; if session.progress.step>3 then return UI.advance(session,2,"The thorns are marked. Match each plant with its safest tool.") end
    ...
    elseif session.phase==2 then
        ...
        session.progress.step=session.progress.step+1; if session.progress.step>3 then return UI.advance(session,3,"The plants are safe. Restore the soil from roots upward.") end
        session.message="Plant treated. Inspect the next one."; return "progress"
    end
    if index~=session.progress.step then return UI.miss(session,"Healthy beds need compost, then water, then mulch.") end
...
function Garden.mousepressed(session,x,y) local i=UI.hit(Garden.zones,x,y); return i and Garden.choose(session,i) end
function Garden.draw(session,colors,image,clock)
    ...
    love.graphics.setColor(.18,.32,.12); love.graphics.rectangle("fill",255,245,450,190,18,18)
    for x=295,665,74 do love.graphics.setColor(.42,.26,.12); love.graphics.rectangle("fill",x,265,38,145,10,10) end
    if session.phase==1 then
        local active=target(session); for i,z in ipairs(Garden.zones) do love.graphics.setColor(i==active and {.72,.92,.25,1} or {.4,.68,.25,.8}); love.graphics.circle("line",z.x,z.y,i==active and 50*pulse or 39); love.graphics.setColor(cream); love.graphics.print(tostring(i),z.x-5,z.y+48) end
    elseif session.phase==2 then
        love.graphics.setColor(cream); love.graphics.printf("CURRENT:  "..current(session).name,270,220,420,"center",0,.78,.78)
        for i,z in ipairs(Garden.zones) do love.graphics.setColor(brass); love.graphics.circle("line",z.x,z.y,55); UI.sprite(image,i,z.x,z.y,92); love.graphics.setColor(cream); love.graphics.printf(i.."  "..toolNames[i],z.x-72,z.y+60,144,"center",0,.58,.58) end
    else
        for i,z in ipairs(Garden.zones) do love.graphics.setColor(brass); love.graphics.circle("line",z.x,z.y,55); UI.sprite(image,4,z.x,z.y,88); love.graphics.setColor(cream); love.graphics.printf(i.."  "..restoration[i],z.x-72,z.y+60,144,"center",0,.58,.58) end
    end
    UI.footer(session,cream,brass,"Protect healthy growth. Three mistakes end the attempt for a safe retry.")
end
function Garden.audit()
    local s=Garden.new({location=2,progress={}}); local ok=true
    for _=1,3 do ok=ok and Garden.choose(s,target(s))=="progress" end
    for _=1,3 do ok=ok and Garden.choose(s,current(s).tool)=="progress" end
    local done; for i=1,3 do done=Garden.choose(s,i) end
    local restored=Garden.new({location=2,progress=s.progress}); local bad=Garden.new({location=1,progress={}}); local failed
    for _=1,3 do failed=Garden.choose(bad,target(bad)%3+1) end
    return {ready=ok and done=="complete" and restored.phase==3 and restored.progress.step==4 and failed=="failed",stages=3,decisions=9,maximumMistakes=3,persistent=true,keyboard=true,touch=true,curve="garden-rescue-v1"}
end
return Garden
