...
- The track uses a fixed `96/181` authored world scale at every aspect ratio. A stable flat rail bed and four ballast pocket frames share the same scroll distance. Draw scales account independently for the actual rail image and ballast frame dimensions, so resized mobile textures preserve the shared rail baseline and tile width without increasing texture sizes.
