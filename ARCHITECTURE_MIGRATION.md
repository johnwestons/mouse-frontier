...
| Legacy module adapters | String-key dependency resolvers using `setfenv` | Explicit context objects and direct module APIs | Session bootstrap, gameplay input, and gameplay update complete |
...
4. Input-owned overlay, transition, interaction, and battle fields now live in runtime state. Remaining inventory, journey, screen, renderer, and HUD adapters move in focused changes rather than one high-risk rewrite.
